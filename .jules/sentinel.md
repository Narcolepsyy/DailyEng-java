...
## 2025-02-23 - Prevent DoS Vulnerabilities by Validating Search Query Length Limits
**Vulnerability:** Endpoints handling internal database search queries (e.g., in `GrammarController`, `VocabController`, `SpeakingController`, `DictionaryController`, `NotificationController`) did not impose constraints on the length of search string parameters (`q`, `query`, or `searchQuery`).
**Learning:** Permitting unbound string parameters in database search queries poses a Denial of Service (DoS) vulnerability. Attackers could submit massive payloads that consume backend memory allocations and burden the database with extremely expensive string operations (like `LIKE` or matching).
**Prevention:** In the Spring Boot backend, explicitly enforce length limits (e.g., `q.length() > 100`) on string parameters for internal search endpoints within controllers prior to delegating the execution to the database query logic.
