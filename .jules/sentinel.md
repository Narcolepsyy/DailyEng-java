...
## 2026-04-14 - Limit Search Query Lengths
**Vulnerability:** Internal REST search endpoints (`VocabController`, `GrammarController`, `SpeakingController`, `DictionaryController`, `NotificationController`) accepted string queries (`q`, `query`, `searchQuery`) used in database `LIKE` matching operations without applying maximum length limits, exposing the system to Denial of Service (DoS) attacks via memory exhaustion and computationally expensive database scans.
**Learning:** External API input bounds are frequently enforced explicitly (as seen in `SpeechController`), but internal search/filter bounds are often overlooked because developers trust that UI limits or frontend behavior will control input size.
**Prevention:** In Spring REST controllers, apply `@Size(max=...)` on DTO attributes or execute manual bounds checking (`q.length() > 100`) on raw `@RequestParam` strings prior to passing data down to the service or repository layers.
