...
## 2024-04-20 - Prevent DoS from unconstrained search queries
**Vulnerability:** Several REST controller endpoints (`/grammar/topics/search`, `/speaking/scenarios/search`, `/dictionary/words/search`, `/dictionary/grammar/search`, `/vocab/topics/search`, `/notifications`) accepted search queries (`q` or `searchQuery`) via `@RequestParam` without any length validation. These strings were passed down to database repositories and used in SQL `LIKE` queries.
**Learning:** Allowing unbound string lengths in search queries can lead to Denial of Service (DoS). Attackers can send excessively large strings that consume memory during allocation and tie up database resources with expensive `LIKE` query evaluations.
**Prevention:** Always enforce explicit length limits (e.g., `q.length() > 100`) on string parameters for search endpoints to prevent massive string allocations and expensive database operations.
