...
## 2024-05-24 - Enforce Length Limits on Search Query Parameters
**Vulnerability:** Multiple REST controllers (Vocab, Grammar, Speaking, Dictionary, Notification) accepted `String` search query parameters (`q` or `searchQuery`) via `@RequestParam` without any explicit length limits.
**Learning:** In Spring Boot applications, unconstrained string inputs mapped to database `LIKE` queries can be exploited by attackers sending massive strings. This causes memory exhaustion (massive string allocation in the JVM) and DoS via expensive database operations.
**Prevention:** Always enforce explicit length limits (e.g., `< 100` characters) on user-provided search strings before processing them or passing them to the database, returning a 400 Bad Request immediately if the limit is exceeded.
