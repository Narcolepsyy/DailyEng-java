...
## 2025-02-24 - [Input Validation] Missing length limits on internal search queries
**Vulnerability:** Controller parameters for internal database search endpoints (e.g., `@RequestParam String q` in VocabController, GrammarController, DictionaryController or `searchQuery` in NotificationController) did not enforce a maximum length. This allowed bad actors to submit massive strings.
**Learning:** Even internal database queries (like `LIKE %query%`) without string length bounds can lead to Denial of Service (DoS) attacks. A massive string parameter will result in huge string allocations causing memory bloat and will trigger highly expensive database operations.
**Prevention:** Explicitly enforce length limits (e.g., `q.length() > 100`) on string parameters for internal search endpoints to prevent DoS vulnerabilities from massive string allocations and expensive database `LIKE` query operations.

## 2025-05-09 - Prevent Information Disclosure / IDOR via DB Layer Authorization
**Vulnerability:** Endpoints fetching a specific entity by its generic ID (e.g., `sessionId`) and checking ownership (e.g., `if (!userId.equals(session.getUserId()))`) in the application layer can leak whether the session inherently exists. If it exists but belongs to another user, they get the same ResourceNotFoundException, but performance timing could vary, or worse, previous code variants threw different errors like UnauthorizedException versus ResourceNotFoundException.
**Learning:** Shifting authorization checks directly into the database query layer (e.g., `repository.findByIdAndUserId(id, userId)`) inherently solves IDOR and Information Disclosure (Observable Response Discrepancy). The query acts as both the lookup and the authorization guard. Unauthorized users and nonexistent records uniformly yield an empty result leading to the same generic `ResourceNotFoundException`.
**Prevention:** Instead of fetching by generic ID (`findById`) and then manually verifying ownership `equals(userId)`, prefer a repository method that fetches by both ID and User ID (`findByIdAndUserId`).
