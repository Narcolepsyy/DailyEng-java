...
## 2026-05-16 - Prevent IDOR in StudyTask Updates
**Vulnerability:** Insecure Direct Object Reference (IDOR) on study task update endpoints (`/tasks/{taskId}/toggle` and `/tasks/{taskId}/time`). Users could toggle or update timestamps on study tasks belonging to other users simply by providing their task IDs, because `findTaskById(taskId)` fetched tasks purely by ID without verifying the ownership of the parent study plan.
**Learning:** The service layer fetched entities by ID (`studyTaskRepository.findById`) and modified them without verifying ownership, exposing them to any authenticated user guessing a valid task ID.
**Prevention:** Shift authorization checks to the database query layer. Use custom repository methods like `findByIdAndPlanUserId(taskId, userId)` to inherently verify ownership during the fetch, ensuring unowned resources return a generic `ResourceNotFoundException`.
