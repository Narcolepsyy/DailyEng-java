...
## 2026-04-09 - Prevent DoS from Massive Search Queries
**Vulnerability:** Several controllers (`VocabController`, `GrammarController`, `SpeakingController`, `DictionaryController`) had internal search endpoints accepting search strings via `@RequestParam String q` without any length validation. This could allow an attacker to send excessively long strings (e.g., millions of characters), causing memory exhaustion on the server or creating expensive database `LIKE` queries that tie up database connections.
**Learning:** Spring Boot's `@RequestParam` does not automatically enforce length limits unless explicitly configured with `@Size` (and `@Validated` on the controller). In a REST API, user inputs mapped directly to database searches must have strict length bounds.
**Prevention:** Always implement explicit string length checks or use validation annotations (`@Size(max=100)`) for search queries and other unbounded string inputs before passing them to the service layer.
