...
## 2026-04-11 - [Input Validation] Missing input length limit on external API controller payloads
**Vulnerability:** The Spring Boot API mapped large string query/form parameters (`targetLanguage`, `searchQuery`, `query`, `q`) and passed them directly to downstream services or resolution functions. This could lead to Denial of Service (DoS) and memory exhaustion.
**Learning:** Even internal helper functions mapping standard types (like search strings and language codes) can cause OutOfMemory errors, DoS attacks, or massive string allocation overheads if bad actors submit payloads with millions of characters to a REST controller that lacks length bounds.
**Prevention:** Add hardcoded string `.length()` checks (e.g., `> 100`) inside the controller at the entry point, or apply standard Spring `@Size(max=...)` annotations to enforce bounds on every string input, regardless of how "harmless" the field seems.
