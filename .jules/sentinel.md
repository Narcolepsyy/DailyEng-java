...
## 2025-02-23 - Prevent DoS from Massive Search Query Allocation
**Vulnerability:** Controller endpoints (e.g., `/vocab/topics/search`) passing unbounded `@RequestParam String q` inputs directly to database `LIKE` query layers allowed attackers to allocate massive strings on the server and execute expensive unbounded wildcard search operations in the database, potentially leading to Memory Exhaustion or Denial of Service (DoS).
**Learning:** Database `LIKE %query%` operations are expensive. When combined with unbounded string input, bad actors can submit excessively large search terms that consume significant server memory during string allocation and severely tax the database CPU during execution.
**Prevention:** Always enforce strict string length limits (e.g., `if (q != null && q.length() > 100) return ResponseEntity.badRequest().build();`) directly in the `@RestController` at the point of entry before passing query strings to internal services or database layers.
