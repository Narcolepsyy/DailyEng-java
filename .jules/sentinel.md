...
## 2025-02-20 - Missing bounds on LIKE queries
**Vulnerability:** Several search endpoints (e.g., `VocabController.searchTopics`, `GrammarController.searchTopics`, `SpeakingController.searchScenarios`, `DictionaryController.searchWords/searchGrammar`, `NotificationController.getNotifications`) passed user-provided string queries (`q` or `searchQuery`) directly to database queries using `LIKE`. Since these strings lacked a length bound, an attacker could send a massive string to execute an expensive database `LIKE` operation, resulting in Memory Exhaustion and Denial of Service (DoS) vulnerabilities on the backend.
**Learning:** Database queries using `LIKE` with wildcards (`%`) are computationally expensive. A missing length limit on internal search endpoints allows attackers to degrade database performance and exhaust server memory with excessively long payloads.
**Prevention:** Explicitly enforce length limits (e.g., `q.length() > 100`) on string parameters for internal search endpoints (`@RequestParam String q` or `searchQuery`) inside the controller prior to calling internal services and executing database operations.
