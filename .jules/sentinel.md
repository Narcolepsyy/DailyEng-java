...
## 2025-02-24 - Enforce Length Limits on Search Query Parameters
**Vulnerability:** Multiple internal search endpoints (`VocabController`, `SpeakingController`, `GrammarController`, `NotificationController`, `DictionaryController`) lacked length validation on user-provided string query parameters (e.g., `q`, `searchQuery`, `query`).
**Learning:** Without explicit length limits, malicious actors can send massive strings (e.g., megabytes in size) in search queries. This causes excessive memory allocation in the JVM and triggers extremely expensive database operations (like `LIKE %query%`), leading to Memory Exhaustion and Denial of Service (DoS). The codebase uses standard `@RequestParam String` without constraints by default.
**Prevention:** Always manually enforce a reasonable maximum length (e.g., `q.length() > 100`) on search strings in controller methods, returning `400 Bad Request` if the limit is exceeded, or use `@Size(max=...)` on DTO fields where applicable.
