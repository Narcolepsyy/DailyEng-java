...
## 2025-02-25 - Prevent DoS from Massive Search Query Strings
**Vulnerability:** Internal search endpoints (like `/vocab/topics/search`, `/speaking/scenarios/search`, and `/grammar/topics/search`) accepted unbounded string inputs for the `@RequestParam String q` parameter. This could allow an attacker to send a massive string (e.g., several megabytes), forcing the server to allocate excessive memory and execute extremely expensive `LIKE` database queries.
**Learning:** Even internal GET requests that don't directly modify data can be vectors for Denial of Service (DoS) and Memory Exhaustion if their inputs are passed directly to database operations without length constraints. The problem exists because `@RequestParam` has no default length limit in Spring Boot.
**Prevention:** Always explicitly enforce length limits (e.g., `q.length() > 100`) on string parameters for internal search endpoints to prevent DoS vulnerabilities from massive string allocations and expensive database `LIKE` query operations. Return an HTTP 400 Bad Request early if the limit is exceeded.
