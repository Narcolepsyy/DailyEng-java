...
## 2024-03-24 - Hoisting Redundant String Allocations Outside Filter Loops
**Learning:** `searchQuery.toLowerCase()` calls within `.filter()` loops are surprisingly expensive in React when dealing with arrays of complex objects, specifically when there are multiple inner loop checks (like `.some()`) within each object iteration. This leads to heavy string memory allocation and $O(N*M)$ string computations.
**Action:** Always pre-calculate invariant derived state properties like lowercase search queries *outside* the `.filter()` block or loop iterator. Store the result in a local `const` variable inside the `useMemo` block, allowing $O(N)$ operations to compare against a stable reference instead of repeatedly computing it per item.
