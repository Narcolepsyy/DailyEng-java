...
## 2024-05-19 - [Consolidated redundant array traversals in ActivityHeatmap]
**Learning:** Found a pattern in `src/components/page/ProfilePageClient.tsx` where derived statistics (`totalLessons`, `activeDays`) were calculated using standalone array methods like `Object.values(data).reduce()` and `Object.values(data).filter(...).length` immediately before a `.forEach` loop over the same dataset. This resulted in multiple unnecessary O(N) traversals and creation of intermediate arrays, increasing garbage collection pressure.
**Action:** Always scan for separate array aggregations/counts that operate on the same data structure and consolidate them into a single existing loop to traverse the data exactly once.
