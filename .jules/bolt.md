...
## 2025-05-19 - Array GC Optimization via Loop Consolidation
**Learning:** Found multiple instances where array length was computed by creating intermediate arrays with `Array.prototype.filter().length` (e.g., `turns.filter(t => t.role === "user").length` or `selectedModule.lessons.filter(l => l.completed).length`), and cases where the same array data was traversed multiple times to compute different stats (e.g. `completedModules` and `totalXP`). This creates unnecessary garbage collection (GC) pressure and performance overhead, particularly in frequently re-rendering React components.
**Action:** Replace `Array.prototype.filter(...).length` with `Array.prototype.reduce((count, item) => condition ? count + 1 : count, 0)` to count elements in-place without array allocations. Whenever computing multiple derivations from an array, use a single `reduce` or `forEach` loop to perform all aggregations in one O(N) pass, completely avoiding multiple redundant sequential loop iterations.
