...
## 2025-05-20 - [Optimized Memory Allocation in Array Filtering/Counting]
**Learning:** Using `Array.prototype.filter(...).length` or `.filter(...)` followed immediately by a manual loop over the filtered array creates an intermediate array allocation. In React states or `useMemo` hooks managing potentially thousands of items (like vocabulary words or flashcards), this creates unnecessary Garbage Collection (GC) overhead and iterates over the array multiple times.
**Action:** Always refactor `.filter(...).length` to an in-place counter via a single `for` or `.reduce()` loop. When iterating to calculate derived statistics (e.g. `getStats`), iterate directly on the source array and use `continue` statements to skip non-matching items rather than creating an intermediate `.filter()` array.
