...
## 2026-04-10 - [Hoisted invariant string operations outside array filters]
**Learning:** In derived state React hooks and components (like `useNotebookUI` and `GrammarPageClient`), calling string methods like `searchQuery.toLowerCase()` *inside* a `.filter()` loop on a large array recalculates the exact same result for every item. In `O(N)` mapping operations, this causes thousands of redundant string allocations, triggering unnecessary garbage collection overhead and significantly slowing down search input responsiveness.
**Action:** Always hoist loop-invariant operations like `.toLowerCase()` or regex compilation outside of the `.filter()` or `.map()` loops. Pre-calculating these values turns O(N) string allocations into a single O(1) allocation, ensuring snappier text-filtering on large data lists.
