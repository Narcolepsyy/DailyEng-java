...
## 2024-05-18 - Optimize Loop-Invariant String Manipulations inside Array Filters
**Learning:** To optimize React render performance when filtering lists (e.g., during searches), always hoist loop-invariant computations like `.toLowerCase()` outside the `.filter()` array iteration. Pre-calculating invariant values prevents redundant string allocations and turns O(N*M) operations into simpler O(N) operations.
**Action:** When filtering an array based on a search string, ensure the search string is converted to lowercase only once outside the loop instead of calling `.toLowerCase()` on the search string for every item in the array.
