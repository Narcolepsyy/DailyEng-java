...
## 2025-05-19 - [Hoisted loop invariants out of filter blocks]
**Learning:** Calling loop-invariant functions like `searchQuery.toLowerCase()` inside a `.filter()` loop, or worse, nested inside `.some()` loops within the filter, forces JavaScript to allocate redundant strings on every iteration. This degrades O(N) operations into O(N*M) constant overhead and causes unnecessary garbage collection pressure, especially during client-side list filtering over large datasets.
**Action:** Always hoist static transformations (like string `.toLowerCase()` on search queries) *outside* the array iteration loop into a constant, significantly optimizing render performance.
