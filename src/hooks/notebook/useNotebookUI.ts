...
export function useNotebookUI({
  ...
  const filteredVocabItems = useMemo(() => {
    // ⚡ Bolt: Hoist toLowerCase() outside the loop to avoid redundant string allocations
    ...
    return vocabularyItems.filter((item) => {
      ...
      const matchesSearch = searchQuery === "" ||
        ...
        item.meaning.some(m => m.toLowerCase().includes(queryLower)) ||
        item.vietnamese.some(v => v.toLowerCase().includes(queryLower))
